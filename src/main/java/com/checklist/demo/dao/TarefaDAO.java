...
/**
 * Classe de acesso a dados (DAO) para a entidade Tarefa. Gerencia as operações de CRUD e os
 * relacionamentos da tarefa com: - Usuário (N:1) - Categoria (N:1) - Tags (N:N via TarefaTag) -
...
public class TarefaDAO {
  ...
  /**
   * Construtor da classe TarefaDAO com injeção de dependências.
   */
  public TarefaDAO(UsuarioDAO usuarioDAO, CategoriaDAO categoriaDAO, 
                   UsuarioTarefasManager usuarioTarefasManager, 
                   CategoriaTarefasManager categoriaTarefasManager,
                   TarefaLogsManager tarefaLogsManager,
                   TarefaTagDAO tarefaTagDAO,
                   LogConclusaoDAO logConclusaoDAO) throws Exception {
    ...
    this.tarefaLogsManager = tarefaLogsManager;
   ...
   */
  private TarefaTagDAO getTarefaTagDAO() {
    return tarefaTagDAO;
  }
  ...
  /**
   * Busca todas as tarefas de um usuário. Utiliza o UsuarioTarefasManager (Hash Extensível).
   *
  ...
  /**
   * Busca todas as tarefas de uma categoria. Utiliza o CategoriaTarefasManager (Hash Extensível).
   *
  ...
  /**
   * Busca tarefas por status (pendente/concluida). Utiliza scan sequencial (não indexado).
   *
  ...
  /**
   * Inclui uma nova tarefa no sistema. Valida usuário e categoria antes de criar.
   *
  ...
  /**
   * Altera os dados de uma tarefa existente. Atualiza os relacionamentos se usuário ou categoria
   * forem alterados.
   *
  ...
  /**
   * Exclui uma tarefa do sistema. Verifica se existem logs ou tags associados antes de excluir.
   *
  ...
  public boolean excluirTarefa(int id) throws Exception {
    ...
    if (numLogs > 0) {
      throw new Exception(
          "Não é possível excluir tarefa pois existem "
              + numLogs
              + " logs de conclusão associados!");
    }
    ...
    if (!tags.isEmpty()) {
      throw new Exception(
          "Não é possível excluir tarefa pois existem " + tags.size() + " tags associadas!");
    }
  ...
  /**
   * Conclui uma tarefa. Atualiza o status para "concluida" e cria um log de conclusão.
   *
   * @param id Identificador da tarefa
